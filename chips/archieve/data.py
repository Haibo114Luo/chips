...
# 底池统计接口：计算所有玩家当前下注的总和
@app.route('/pot', methods=['GET'])
def get_pot():
    players = Player.query.all()
    total_pot = sum(player.current_amount for player in players)
    return jsonify({'pot': total_pot}), 200

# 更新玩家下注接口：修改当前下注金额（用于底池统计）
...
def update_bet():
   ...
   # 使用current_amount代替不存在的current_bet属性
    player.current_amount = current_bet
    db.session.commit()
